...
def get_web_context(query):
    try:
        ...
        payload = {"q": query + " facts explanation", "num": 4}
        headers = {
...
def build_prompt(state: State):
    ...
    if no_paper:
        paper_status = "⚠️ NO PAPER LOADED."
        paper_instruction = (
            "Since no paper is loaded, warmly greet the user and explain they can upload "
            "a PDF on the left side or resume a discussion from the Library. Keep it short and friendly."
        )
    else:
...
Instructions:
1. Explain complex concepts clearly and conversationally.
2. Use Markdown (bolding, bullet points, headers) for readability.
3. Be concise — user may be listening via TTS.
4. Academic Focus: Strictly Computer Science / Research domain.
5. {paper_instruction}

Active Document: {paper_status}
...
def generate(state: State):
    ...
    history = "\n".join(history.split("\n")[-20:])
    return {"ans": response, "history": history}
